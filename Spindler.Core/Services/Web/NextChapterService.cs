using Spindler.Models;
using Spindler.Utilities;

namespace Spindler.Services.Web;

public class NextChapterService(HttpClient client, IWebService browser, IDataService database)
{
    private readonly HttpClient _Client = client;
    private readonly IWebService _WebService = browser;
    private readonly IDataService _Database = database;

    public async Task SaveBooks(IEnumerable<Book> books, CancellationToken token)
    {
        foreach (Book book in books)
        {
            Book updated = await CheckNextChapterBook(book, token);
            await _Database.SaveItemAsync(updated);
            if (token.IsCancellationRequested)
            {
                return;
            }
        }
    }

    public async Task<Book> CheckNextChapterBook(Book book, CancellationToken token)
    {

        if (book.HasNextChapter)
        {
            return book;
        }

        Result<string> result = await _WebService.GetHtmlFromUrl(book.Url, token);

        if (result is Result<string>.Err)
        {
            return book;
        }

        string html = (result as Result<string>.Ok)!.Value;
        Config? config = await Config.FindValidConfig(_Database, _Client, book.Url, html);

        if (config is null || config.UsesWebview)
            return book;

        string nextUrl = config.ContentPath.AsPath().Select(html, SelectorType.Link);
        book.HasNextChapter = UrlBuilder.IsUrl(nextUrl);
        return book;
    }

    /// <summary>
    /// Starts an Independent Task that will Check Books until completed, or get cancelled by the CancellationToken
    /// </summary>
    /// <param name="books">A list of Book objects to check for the next chapter</param>
    /// <param name="cancellationToken">Cancellation Token</param>
    /// <returns>A task that represents the thread being used to check for books</returns>
    public Task StartIndependentCheckingThread(IEnumerable<Book> books, CancellationToken cancellationToken)
    {
        return Task.Run(() => SaveBooks(books, cancellationToken), CancellationToken.None);
    }
}
